...
import { handleBannerDelete } from "@/app/actions";
...
export function BannersTable({ data }: BannersTableProps) {
  const { toast } = useToast();
  const [isDeleting, setIsDeleting] = useState(false);

  const onDelete = async (bannerId: string) => {
    setIsDeleting(true);
    const result = await handleBannerDelete(bannerId);
    if (result.success) {
      toast({ title: "Success", description: "Banner deleted successfully." });
    } else {
      toast({
        title: "Error",
        description: result.message || "Failed to delete banner.",
        variant: "destructive",
      });
    }
    setIsDeleting(false);
